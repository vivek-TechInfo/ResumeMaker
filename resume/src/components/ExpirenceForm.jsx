import { Briefcase, Loader2, Plus, SparklesIcon, Trash2 } from "lucide-react";
import React, { useState } from "react";
import { useSelector } from "react-redux";
import api from "../configs/api";
import toast from "react-hot-toast";

const ExpirenceForm = ({ data, onChange }) => {
  ...
  const generatingDescription = async(index)=>{
    ...
    const expirence = data[index]

    const prompt = `enhance this job description ${expirence.description} for the position of ${expirence.position} at ${expirence.company} `
    ...
    try {
      ...
      updateExperience(index,"description",data.enchanceContent)
    ...
    <div className="space-y-6 ">
      <div className="flex items-center justify-between">
        <div className="">
          ...
          <p className="text-sm text-gray-500">Add your job expirence</p>
        </div>
                    ...
                    <div key={index} className="p-4 border border-gray-200 rounded-lg space-y-3">
                        ...
                        <div className="space-y-2">
                            <div className="flex items-center justify-between">
                                ...
                                <button disabled={generatingIndex === index || !experience.position || !experience.company} onClick={()=>generatingDescription(index)} className="flex items-center gap-1 px-2 py-1 text-xs  bg-purple-100 text-purple-700 rounded hover:bg-purple-200 transition-colors disabled:opacity-50">
                                  ...
                                  {
                                    generatingIndex=== index ? "Enhancing..." :"Enchance with AI"
                                  }
                                    
                                </button>

                            </div>
                            <textarea value={experience.description || ""} onChange={(e)=>updateExperience(index,"description" , e.target.value)} rows={4} name="" id="" className="w-full text-sm px-3 py-2 rounded-lg resize-none" placeholder="Describe your key responsibilities and achievements..."/>
