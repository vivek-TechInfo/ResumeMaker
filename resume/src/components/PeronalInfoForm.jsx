import { BriefcaseBusiness, Globe, Linkedin, Mail, MapIcon, Phone, User } from 'lucide-react'
import React from 'react'

const PeronalInfoForm = ({data,  onChange , removeBackground, setRemoveBackground}) => {
        ...
        <div className='flex items-center gap-2'>
            <label htmlFor='image'>
                ...
                <div className='inline-flex items-center gap-2 text-slate-600 hover:text-slate-700 cursor-pointer'>
                    <User className='size-10 p-2.5 mt-2 border rounded-full' />
                    Upload user image
            ...
            <input type="file" accept='image/jpeg, image/png' id='image' className='hidden' onChange={(e)=>handleChange("image", e.target.files[0])} />
            ...
            fields.map((field)=>{
                ...
                return(
                    <div key={field.key} className='space-y-1 mt-5'>
                        ...
                        <input type={field.type} value={data[field.key] || ""} onChange={(e)=>handleChange(field.key , e.target.value)} className='mt-1 w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 outline-none transition-colors text-sm' placeholder={`Enter your ${field.label.toLowerCase()}`} required={field.required} />
...
export default PeronalInfoForm